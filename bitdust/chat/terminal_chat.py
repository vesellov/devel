...
class TerminalChat(object):
    ...
    def run(self):
        ...
        # bot.start()
        out = threading.Thread(target=self.collect_output, daemon=True, name='terminal_chat_collect_output')
        out.start()
        inp = threading.Thread(target=self.collect_input, daemon=True, name='terminal_chat_collect_input')
        inp.start()
        proc = threading.Thread(target=self.process_input, daemon=True, name='terminal_chat_process_input')
        proc.start()
        proc.join()
        self.goodbye()
        try:
