...
class QueueKeeper(automat.Automat):
    ...
    def doRememberCooperation(self, event, *args, **kwargs):
        ...
        """
        if _Debug:
            lg.args(_DebugLevel, event=event, cur=self.cooperated_brokers, new=kwargs.get('cooperated_brokers'))
    ...
    def doRememberOwnPosition(self, *args, **kwargs):
        ...
        for pos, idurl in self.cooperated_brokers.items():
            if idurl and idurl == self.broker_idurl:
                my_new_position = pos
        if _Debug:
            lg.args(_DebugLevel, known_position=self.known_position, my_new_position=my_new_position)
...
#         self.automat('other-broker-disconnected', desired_position=desired_position)
    ...
    def _on_broker_negotiator_callback(self, event, cooperated_brokers):
        if _Debug:
            lg.args(_DebugLevel, event=event, cooperated_brokers=cooperated_brokers)
        self.negotiator = None
